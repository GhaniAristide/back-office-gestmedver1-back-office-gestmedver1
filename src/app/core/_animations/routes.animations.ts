import {
  ...
  const STEPS_ALL: any[] = [
    query(':enter > *:not(.' + ROUTE_ANIMATIONS_ELEMENTS + ')', style({ opacity: 0, position: 'fixed' }), {
      optional: true
    }),
    ...
    sequence([
      query(
        ':leave > *:not(.' + ROUTE_ANIMATIONS_ELEMENTS + ')',
        [
      ...
      query(
        ':enter > *:not(.' + ROUTE_ANIMATIONS_ELEMENTS + ')',
        [
          style({
  ...
  export const routeAnimations = trigger('routeAnimations', [
    ...
    transition(isRouteAnimationsElements, STEPS_NONE)
  ]);
